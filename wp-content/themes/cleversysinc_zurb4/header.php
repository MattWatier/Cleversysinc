...
<header role="banner" id="top-header" class="">
		<div class="row">		
				<nav id="top-nav" class="small-12 columns">
...
<div id="product_collection" class="row">
	<div class="section-container tabs" data-section="tabs">
	<?php	$args = array('taxonomy' => 'custom_product_cat' );  
				$custom_cat =  get_categories( $args );
				$menu ='<section class="section"><div class="content">&nbsp;</div><p class="title"><a href="#">Close</a></p></section>';
				
				foreach ($custom_cat as $product) {
					$menu.= '<section class="section">';
					$menu .= '<p class="title"><a href="#'.$product->slug.'">'.$product->name.'</a></p>';
					$menu.=	'<div id="'.$product->slug.'Tab"  class="content"><ul class="small-block-grid-2 large-block-grid-4">';
					$args = array(
					   'numberposts' => -1,
					...
					foreach($links as $link){
						$linkList .='<li><a href="'.get_bloginfo(wpurl).'/?'.$link->post_type.'='.$link->post_name.'#blank">'.$link->post_title.'</a></li>';
					}
